...
// 객체가 만들어지는 함수render에 color 인자를 전달받게 하였음
void Scene::render(Graph_lib::Color color)
{
	clear();
	...
	for (int i = 0; i < (int)mesh_list.size(); i++)
	{
		...
		Graph_lib::Lines* lines = mesh->render(window, mat_camera, is_culling_enabled, color);  //mesh만들기 시작!
